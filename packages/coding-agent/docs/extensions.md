...
`ctx.scopedModels` is the read-only intersection of the configured scope and the currently available catalogue—the same model set shown by the scoped tab of `/model`. CLI `--models` patterns are resolved once when the process starts and then retained as exact identities across session replacement. Persisted `enabledModels` entries are exact `provider/model` identities and are re-resolved as provider catalogues change. An empty list can therefore mean either that scoping is disabled or that an explicit scope currently has no available models. Each entry is `{ model, thinkingLevel? }`. Use it to populate a model picker that mirrors the built-in scoped tab instead of enumerating the whole catalogue via `ctx.modelRegistry.getAvailable()`.
